...
pub fn run(_cmd: UnlockCmd, vault_path: &str, json: bool) -> anyhow::Result<()> {
    let path = resolve_vault_path(vault_path);
    let storage = FilesystemBackend::new(path.clone());

    if !storage.vault_exists()? {
        anyhow::bail!("vault not found at {}. Run `padlock init` first.", path.display());
    }
    ...
    if let (Ok(kek), Ok(mackey)) = (vault.kek(), vault.mackey()) {
        ...
        if let Some(token) = super::create_daemon_session(
            ...
            mackey,
            &vault_id,
            padlock_core::session::types::SessionDuration::OneHour,
        ) {
